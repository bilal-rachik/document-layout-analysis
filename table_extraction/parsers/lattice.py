...
class Lattice(object):
    ...
    def _generate_table_bbox(self):
        ...
        self.table_bbox, self.vertical_segments, self.horizontal_segments = translate_image(
            table_bbox, vertical_segments, horizontal_segments, self.image_height
        )
    ...
    def _remove_lines(self):
        lines = self.vertical_mask + self.horizontal_mask
    ...
    def get_text_table(self, tk):
        x1, y2, x2, y1 = tk
        image_table = self.image_clean[self.image_height- y1: self.image_height - y2, x1:x2]
        results = pytesseract.image_to_data(image_table, output_type=Output.DICT)
        t_box = get_text_box(results, x1, self.image_height - y1, self.image_height)
        return t_box
    ...
    def _generate_table(self, table_idx, cols, rows, **kwargs):
        ...
        # for plotting
        _text = []
        _text.extend([(t.x0, t.y0, t.x1, t.y1) for t in self.t_bbox])
        table._text = _text
    ...
    def extract_tables(self, filename):
        ...
        self.image_clean = self._remove_lines()
        ...
        for table_idx, tk in enumerate(
            ...
            self.t_bbox = self.get_text_table(tk)
